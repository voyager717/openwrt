...
static int wapp_cli_cmd_version(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "interface=%s\n", argv[0]);
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
	i += 11 + os_strlen(argv[0]);
	
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=hs_version\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_on(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=on\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_off(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=off\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_set(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (match) {
		...
		ret2 = snprintf(&cmd[i],
			sizeof(cmd) - i, "cmd=set %s %s", argv[2], argv[3]);
		if (ret2 < 0) {
			DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
			return -1;
		}
...
static int wapp_cli_cmd_btmreq(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i,
		"cmd=btmreq %s %s %s %s", argv[2], argv[3], argv[4], argv[5]);
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_ext(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	i += 11 + os_strlen(argv[0]);
#if 1	
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
	...
	for(j = 1; j < argc; j++) {
		ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "%s ", argv[j]);
		if (ret2 < 0) {
			DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
			return -1;
		}
...
#else	
	snprintf(&cmd[i], sizeof(cmd) - i, "cmd=%s %s %s %s %s", argv[1], argv[2], argv[3], argv[4], argv[5]);
#endif
...
static int wapp_cli_cmd_drv_version(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=drv_version\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_wps_pbc_ctrl(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=wps_pbc_trigger\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_get_version(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=version_info\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_wps_PbcOnCli_ctrl(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	i += 11 + os_strlen(argv[0]);
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=wps_pbc_trigger_cli\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
												...
												int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=ipv4_proxy_arp_list\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
												...
												int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=ipv6_proxy_arp_list\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
								  ...
								  int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i], sizeof(cmd) - i, "cmd=reload\n");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_wnmreq(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	ret2 = snprintf(&cmd[i],
		sizeof(cmd) - i, "cmd=wnmreq %s 0 %s", argv[2], argv[3]);
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
static int wapp_cli_cmd_wnmreq2(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc == 5)
		ret2 = snprintf(&cmd[i], sizeof(cmd) - i,
			"cmd=wnmreq %s 1 %s %s", argv[2], argv[3], argv[4]);
	else
		ret2 = snprintf(&cmd[i],
			sizeof(cmd) - i, "cmd=wnmreq %s 1 %s %s %s",
...
static int wapp_cli_cmd_qosmap(struct wapp_ctrl *ctrl, int argc, char *argv[])
{
	...
	if (argc == 5)
		ret2 = snprintf(&cmd[i], sizeof(cmd) - i,
			"cmd=qosmap %s 0 %s %s", argv[2], argv[3], argv[4]);
	else
		ret2 = snprintf(&cmd[i], sizeof(cmd) - i,
			"cmd=qosmap %s 1 %s", argv[2], argv[3]);
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "snprintf error ");
		return -1;
	}
...
#ifdef QOS_R1
	{"qm cmd [arg1] [arg2]", wapp_cli_cmd_ext, "qos cmd"},
#endif
};
...
int main(int argc, char *argv[])
{
	...
	ret2 = os_snprintf(socket_path, sizeof(socket_path), "/tmp/wapp_ctrl");
	if (ret2 < 0) {
		DBGPRINT(RT_DEBUG_ERROR,
			"Failed to open connection to wapp socket path ");
		return ret2;
	}
