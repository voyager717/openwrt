...
extern const struct dpp_curve_params dpp_curves[];
				     ...
				     struct json_token *peer_net_access_key)
{
	...
	if(peer_key) {
		...
		auth->peer_connector_key = peer_key;
	}
						...
						struct dpp_reconfig_id *id)
{
	...
	wpa_printf(MSG_ERROR, "DPP: Build Reconfig Announcement frame");
...
struct wpabuf * dpp_reconfig_build_req(struct dpp_authentication *auth)
{
	...
	if (auth->conf->connector == NULL)
		wpa_printf(MSG_DEBUG, "\n DPP: auth->conf->connector is NULL");
	...
	hex_dump_dbg("nonce dump= ",(UCHAR *)auth->c_nonce, auth->curve->nonce_len);
		  ...
		  const u8 *e_id_attr, size_t e_id_len)
{
	...
	if (!e_id_attr) {
		wpa_printf(MSG_DEBUG, "DPP: Missing required E'-id attribute");
		return NULL;
	}
	e_prime_id = dpp_set_pubkey_point(conf->csign, e_id_attr, e_id_len);
	if (!e_prime_id) {
		wpa_printf(MSG_DEBUG, "DPP: Invalid E'-id");
	...
	auth->allowed_roles = dpp->dpp_allowed_roles;
	auth->configurator = !!(dpp->dpp_allowed_roles & DPP_CAPAB_CONFIGURATOR);
	auth->peer_bi = NULL;
	auth->own_bi = own_bi;
...
#endif /* DPP_R2_STATUS */
#if 0
	if (freq && dpp_prepare_channel_list(auth) < 0)
		goto fail;
#endif
			 ...
			 const u8 *attr_start, size_t attr_len)
{
	...
	if (dpp_prepare_channel_list(auth) < 0)
		goto fail;
	...
	if (!conn_status)
       ...
       build_resp = dpp_reconfig_build_resp(auth, own_connector, conn_status);
	if (!build_resp)
		goto fail;
...
struct wpabuf *
dpp_reconfig_auth_resp_rx(struct dpp_authentication *auth, struct wifi_app *wapp, const u8 *hdr,
			 const u8 *attr_start, size_t attr_len)
{
	...
	trans_id = dpp_get_attr(attr_start, attr_len, DPP_ATTR_TRANSACTION_ID,
			       &trans_id_len);
	if (!trans_id || trans_id_len != 1) {
		dpp_auth_fail_wrapper(wapp, "Peer did not include Transaction ID");
		goto fail;
	}
	if (trans_id[0] != auth->transaction_id) {
		dpp_auth_fail_wrapper(wapp, "Transaction ID mismatch");
		goto fail;
	}
	...
	r_connector = dpp_get_attr(attr_start, attr_len, DPP_ATTR_CONNECTOR,
				   &r_connector_len);
	if (!r_connector) {
		dpp_auth_fail_wrapper(wapp, " Missing R-Connector attribute");
		goto fail;
	}
	...
	if (!e_nonce || e_nonce_len != auth->curve->nonce_len) {
		dpp_auth_fail_wrapper(wapp, "Missing or invalid E-nonce");
		goto fail;
	}
	...
	res = dpp_process_signed_connector(&info, auth->conf->csign,
					   signed_connector);
	if (res != DPP_STATUS_OK) {
		dpp_auth_fail_wrapper(wapp, "Invalid R-Connector");
		goto fail;
	}

	root = json_parse((const char *) info.payload, info.payload_len);
	if (!root) {
		dpp_auth_fail_wrapper(wapp, "Invalid Connector payload");
		goto fail;
	}
	...
	token = json_get_member(root, "netAccessKey");
	if (!token || token->type != JSON_OBJECT) {
		dpp_auth_fail_wrapper(wapp, "No netAccessKey object found");
		goto fail;
	}
	...
	if (aes_siv_decrypt(auth->ke, auth->curve->hash_len,
		...
		dpp_auth_fail_wrapper(wapp, "AES-SIV decryption failed");
		goto fail;
	}
	...
	if (dpp_check_attrs(unwrapped, unwrapped_len) < 0) {
		dpp_auth_fail_wrapper(wapp, "Invalid attribute in unwrapped data");
		goto fail;
	}
	...
	if (!c_nonce || c_nonce_len != auth->curve->nonce_len ||
		...
		dpp_auth_fail_wrapper(wapp, "Missing or invalid C-nonce");
	...
	if (!conn_status) {
		dpp_auth_fail_wrapper(wapp, "Missing Connection Status attribute");
		goto fail;
	}
	...
	conn_status_json = json_parse((const char *) conn_status,
				      conn_status_len);
	if (!conn_status_json) {
		dpp_auth_fail_wrapper(wapp, "Could not parse connStatus");
		goto fail;
	}
			      ...
			      const u8 *attr_start, size_t attr_len)
{
	...
	if (!reconfig_flags || reconfig_flags_len < 1) {
		dpp_auth_fail_wrapper(wapp, "Missing or invalid Reconfig-Flags");
		goto fail;
	}
