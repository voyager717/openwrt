...
static void random_mix_pool(const void *buf, size_t len)
{
	...
	wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "random_mix_pool", buf, len);
...
void random_add_randomness(const void *buf, size_t len)
{
	...
	os_get_time(&t);
	wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "random pool",
...
int random_get_bytes(void *buf, size_t len)
{
	...
	wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "random from os_get_random",
			buf, len);
	...
	while (left) {
		...
		u8 tmp[EXTRACT_LEN];
		random_extract(tmp);
		wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "random from internal pool",
				tmp, sizeof(tmp));
	...
	while (left) {
		...
		if (crypto_get_random(tmp, sizeof(tmp)) < 0) {
			wpa_printf(MSG_ERROR, DPP_MAP_PREX "random: No entropy available "
				   "for generating strong random bytes");
			return -1;
		}
		wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "random from crypto module",
				tmp, sizeof(tmp));
		siz = left > EXTRACT_LEN ? EXTRACT_LEN : left;
		for (i = 0; i < siz; i++)
			*bytes++ ^= tmp[i];
	...
	wpa_hexdump_key(MSG_EXCESSIVE, DPP_MAP_PREX "mixed random", buf, len);
...
int random_pool_ready(void)
{
	...
	if (fd < 0) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "random: Cannot open /dev/random: %s",
			   strerror(errno));
		return -1;
	}
	...
	if (res < 0) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "random: Cannot read from /dev/random: "
			   "%s", strerror(errno));
		res = 0;
	}
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Got %u/%u bytes from "
	...
	wpa_printf(MSG_INFO1, DPP_MAP_PREX "random: Only %u/%u bytes of strong "
		   "random data available from /dev/random",
		   (unsigned) dummy_key_avail, (unsigned) sizeof(dummy_key));

	if (own_pool_ready >= MIN_READY_MARK ||
	    total_collected + 10 * own_pool_ready > MIN_COLLECT_ENTROPY) {
		wpa_printf(MSG_INFO1, DPP_MAP_PREX "random: Allow operation to proceed "
			   "based on internal entropy");
		return 1;
	}

	wpa_printf(MSG_INFO1, DPP_MAP_PREX "random: Not enough entropy pool available for "
		   "secure operations");
	return 0;
#else /* __linux__ */
...
{
	own_pool_ready++;
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Mark internal entropy pool to be "
...
static void random_read_fd(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Got %u/%u bytes from /dev/random",
		   (unsigned) res,
		   (unsigned) (sizeof(dummy_key) - dummy_key_avail));
	dummy_key_avail += res;
...
static void random_read_entropy(void)
{
	...
	if (len != 1 + RANDOM_ENTROPY_SIZE) {
		wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Invalid entropy file %s",
			   random_entropy_file);
		os_free(buf);
		return;
	}
	...
	os_free(buf);
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Added entropy from %s "
		   "(own_pool_ready=%u)",
...
static void random_write_entropy(void)
{
	...
	if (f == NULL) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "random: Could not open entropy file %s "
	...
	if (fail) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "random: Could not write entropy data "
			   "to %s", random_entropy_file);
		return;
	}

	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Updated entropy file %s "
		   "(own_pool_ready=%u)",
		   random_entropy_file, own_pool_ready);
}
...
void random_init(const char *entropy_file)
{
	...
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "random: Trying to read entropy from "
		   "/dev/random");
