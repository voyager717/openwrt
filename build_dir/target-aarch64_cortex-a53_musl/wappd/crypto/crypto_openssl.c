				 ...
				 const u8 *addr[], const size_t *len, u8 *mac)
{
	...
	if (!EVP_DigestInit_ex(ctx, type, NULL)) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_DigestInit_ex failed: %s",
	...
	for (i = 0; i < num_elem; i++) {
		if (!EVP_DigestUpdate(ctx, addr[i], len[i])) {
			wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_DigestUpdate "
				   "failed: %s",
				   ERR_error_string(ERR_get_error(), NULL));
			EVP_MD_CTX_free(ctx);
			return -1;
	...
	if (!EVP_DigestFinal(ctx, mac, &mac_len)) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_DigestFinal failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
		EVP_MD_CTX_free(ctx);
		return -1;
	}
...
void * aes_encrypt_init(const u8 *key, size_t len)
{
	...
	if (!type) {
		wpa_printf(MSG_INFO1, DPP_MAP_PREX "%s: Unsupported len=%u",
...
int aes_encrypt(void *ctx, const u8 *plain, u8 *crypt)
{
	...
	if (EVP_EncryptUpdate(c, crypt, &clen, plain, 16) != 1) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_EncryptUpdate failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
	return 0;
...
void aes_encrypt_deinit(void *ctx)
{
	...
	if (EVP_EncryptFinal_ex(c, buf, &len) != 1) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_EncryptFinal_ex failed: "
			   "%s", ERR_error_string(ERR_get_error(), NULL));
	}
	if (len != 0) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: Unexpected padding length %d "
			   "in AES encrypt", len);
	}
...
int aes_decrypt(void *ctx, const u8 *crypt, u8 *plain)
{
	...
	if (EVP_DecryptUpdate(c, plain, &plen, crypt, 16) != 1) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_DecryptUpdate failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
	return 0;
...
void aes_decrypt_deinit(void *ctx)
{
	...
	if (EVP_DecryptFinal_ex(c, buf, &len) != 1) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_DecryptFinal_ex failed: "
			   "%s", ERR_error_string(ERR_get_error(), NULL));
	}
	if (len != 0) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: Unexpected padding length %d "
...
struct crypto_ecdh * crypto_ecdh_init(int group)
{
	...
	if (EVP_PKEY_keygen(kctx, &ecdh->pkey) != 1) {
		wpa_printf(MSG_ERROR, DPP_MAP_PREX "OpenSSL: EVP_PKEY_keygen failed: %s",
			   ERR_error_string(ERR_get_error(), NULL));
		goto fail;
	}
