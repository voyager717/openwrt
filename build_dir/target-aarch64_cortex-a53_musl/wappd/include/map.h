...
#define MAPD_WTS_FILE "/etc/map/wts_bss_info_config"
#define MAPD_USER_CFG_FILE "/etc/map/mapd_user.cfg"
#ifdef DPP_R2_SUPPORT
#define DPP_MUD_URL_FILE "/etc/dpp_mud_url.txt"
#endif /* DPP_R2_SUPPORT */
#define DPP_CFG_FILE_TEST "/etc/dpp_cfg_test.txt"
...
#define DEV_TYPE_R1 1
#define DEV_TYPE_R2 2
#define DEV_TYPE_R3 3
...
typedef struct _DFS_REPORT_AVALABLE_CH_LIST {
	u8 Channel;
	u8 RadarHitCnt;
} DFS_REPORT_AVAILABLE_CH_LIST, *PDFS_REPORT_AVAILABLE_CH_LIST;
...
union dfs_zero_wait_msg {
	struct _aval_channel_list_msg{
		...
		u8 Bw20TotalChNum;
		u8 Bw160TotalChNum;
		DFS_REPORT_AVAILABLE_CH_LIST Bw80AvalChList[DFS_AVAILABLE_LIST_CH_NUM];
		DFS_REPORT_AVAILABLE_CH_LIST Bw40AvalChList[DFS_AVAILABLE_LIST_CH_NUM];
		DFS_REPORT_AVAILABLE_CH_LIST Bw20AvalChList[DFS_AVAILABLE_LIST_CH_NUM];
		DFS_REPORT_AVAILABLE_CH_LIST Bw160AvalChList[DFS_AVAILABLE_LIST_CH_NUM];
...
#define IS_OP_CLASS_5GL(__op_class) \
	((__op_class > 110 && __op_class <= 120) || (__op_class == 128 || __op_class == 129))
...
#ifdef MAP_320BW
#ifdef MAP_6E_SUPPORT
#define IS_OP_CLASS_6G(__op_class) \
	((__op_class > 130) &&  __op_class <= 137)
#endif
#else
#ifdef MAP_6E_SUPPORT
#define IS_OP_CLASS_6G(__op_class) \
	((__op_class > 130 && __op_class <= 136))
#endif
#endif
...
#define OP_CLASS_MATCH_RADIO_CONF(_radio_band, _op_class) \
	((_radio_band == RADIO_24G && IS_OP_CLASS_24G(_op_class)) || \
	 ...
	 (_radio_band == RADIO_5GH && IS_OP_CLASS_5GH(_op_class)) || \
	 (_radio_band == RADIO_5G && IS_OP_CLASS_5G(_op_class)) || \
	 (_radio_band == RADIO_6G && IS_OP_CLASS_6G(_op_class)))
...
void update_primary_ch_status_opclass(struct wifi_app *wapp, unsigned char channel,
...
int map_config_channel_setting_msg(struct wifi_app *wapp, char *msg_buf, char *evt_buf, int *buf_len);
int map_config_backhaul_connect_msg(struct wifi_app *wapp, char *msg_buf, char *evt_buf, int *len_buf);
#ifdef MAP_6E_SUPPORT
