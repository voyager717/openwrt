...
#else
#define MAX_CH_NUM 59 /* 6E case: Max ch will be 59 */
#endif
...
/*
 * Kindly add command at the end of this enum,
 * before this "WAPP_MAP_MAX_EVENT".
 * WARNING: ADDING COMMAND IN BETWEEN WILL BRAKE THE
 * COMPATIBILITY WITH OLDER VERSION OF CODE.
 * ALSO IN REVIEW YOU W'NOT BE GRANTED TO SUBMIT.
 */
/*WAPP EVENT*/
enum {
	...
	WAPP_WTS_CONFIG,
	WAPP_SEND_URI_MSG,
	WAPP_SEND_CCE_MSG, //Prakhar
	WAPP_SEND_1905_CONNECTOR, //Milan
	WAPP_SEND_BSS_CONNECTOR,
	WAPP_ASSOC_WIFI6_STA_STATUS, //MAP R3
	WAPP_AP_WF6_CAPABILITY,
	WAPP_SEND_CHIRP_MSG,
...
enum {
	...
	WAPP_USER_SET_SERVICE_PRIORITIZATION_RULE,
	WAPP_USER_SET_DSCP_MAPPINT_TABLE,
	WAPP_USER_SEND_DPP_FRAME,
	WAPP_USER_SEND_DPP_CCE_INDICATION_FRAME,
	WAPP_USER_GET_ASSOC_WIFI6_STA_STATUS,
	WAPP_USER_GET_AP_WF6_CAPABILITY,
	WAPP_USER_SEND_AUTOCONFIG_TRIGGER,
	WAPP_USER_SEND_DPP_DIRECT_FRAME,
	WAPP_USER_SEND_CHIRP_TLV_FRAME,
	WAPP_USER_SEND_DPP_URI_NOTIFY_FRAME,
	WAPP_USER_SEND_1905_SEC_NOTIFY_FRAME,
	WAPP_USER_SEND_DPP_INIT_NOTIFY_FRAME,
	WAPP_USER_SEND_TOPO_NOTIFY_FRAME,
	WAPP_USER_SEND_1905_ONBOARD_NOTIFY,
	WAPP_USER_GET_DEV_INVEN_TLV,
	WAPP_SET_SEND_BUFFER_INCR,
	WAPP_USER_GET_RECONFIG_TRIGGER,
	WAPP_USER_SET_RECONFIG_START,
	WAPP_SEND_BUFF_INCR_EVT,
	WAPP_USER_GET_AP_SPT_REUSE,
	WAPP_USER_SET_AP_SPT_REUSE,
	WAPP_USER_SET_AP_SR_BM,
	WAPP_USER_SET_SR_TOPO_INFO,
	WAPP_USER_SET_UPLINK_TRAFFIC_STATUS,
	WAPP_USER_GET_DPP_URI,
...
typedef enum {
	WAPP_LOW_POWER_NO_STA_CONNECT_TIMEOUT = 0,
	WAPP_LOW_POWER_NO_DATA_TRAFFIC_TIMEOUT,
	WAPP_LOW_POWER_WIFI_ON,
	WAPP_LOW_POWER_WIFI_OFF,
} WAPP_LOW_POWER_STATUS;

struct GNU_PACKED low_power_status
{
...
struct GNU_PACKED ap_spt_reuse_resp {
	...
	unsigned char srg_bss_color_bitmap[SRG_BITMAP_SIZE];
	unsigned char srg_partial_bssid_bitmap[SRG_BITMAP_SIZE];
	unsigned char ngh_bss_color_in_bitmap[SRG_BITMAP_SIZE];
};
...
struct srg_topology_info
{
	u8 map_dev_cnt;
	u8 map_dev_sr_mode;
	u8 self_mode;
	u8 srg_remote_info_cnt;
	u8 map_remote_almac[MAC_ADDR_LEN];
	u8 map_remote_bh_mac[MAC_ADDR_LEN];
	struct srg_remote_info srg_info[0];
};
#endif
...
#ifdef MAP_R4_SPT
	unsigned char spt_rep_num;
	struct ap_spt_reuse_resp spt_reuse[3];
#endif
...
struct GNU_PACKED ap_spt_reuse {
	u8 spt_req_cnt;
	struct ap_spt_reuse_req spt_reuse_req[0];
};

struct GNU_PACKED uplink_traffic_status {
	u8 band;
	u8 status;
};
#endif
 ...
 */
struct GNU_PACKED assoc_wifi6_sta_status_tlv {
        unsigned char mac[MAC_ADDR_LEN];
 ...
 * @stats: sta traffic stats info
 */
struct GNU_PACKED assoc_wifi6_sta_status {
        unsigned char identifier[MAC_ADDR_LEN];
        unsigned char sta_cnt;
        struct assoc_wifi6_sta_status_tlv status[0];
};
#endif
...
struct GNU_PACKED interface_info {
	...
	unsigned char if_opclass;/* ToDo: Need to add in 1905 as well */
#endif
...
struct GNU_PACKED dpp_tlv_info {
	...
	unsigned char enrollee_mac[MAC_ADDR_LEN];
...
struct GNU_PACKED chirp_tlv_info {
	...
	unsigned char hash_payload[SHA256_MAC_LEN];
};
...
struct GNU_PACKED dpp_direct_msg
{
	unsigned char almac[MAC_ADDR_LEN];
	unsigned short payload_len;
	unsigned char payload[0];
...
typedef struct GNU_PACKED conn_fail_notif {
	char reason[64];
}conn_fail_reason;
enum user_fail_reason_ids {
	DPP_QR_PARSING_FAIL,
	VERSION_MISMATCH,
	CH_NOT_VALID,
	PEER_DISC_FAIL,
};
typedef struct GNU_PACKED user_fail_notif {
