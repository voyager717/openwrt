...
static inline int hs_param_setting(struct wifi_app *wapp,
							...
							const char *iface)
{
	...
	dl_list_for_each(conf, &wapp->conf_list, struct wapp_conf, list) {
		if (os_strcmp(conf->iface, iface) == 0) {
			is_found = 1;
			res = os_snprintf(confname, sizeof(confname), "%s", conf->confname);
			if (os_snprintf_error(sizeof(confname), res)) {
								   ...
								   unsigned char IsDAD)
{
	...
	*/
		...
		DBGPRINT(RT_DEBUG_TRACE, "Send Proxy ARP Packet to br-lan\n");
