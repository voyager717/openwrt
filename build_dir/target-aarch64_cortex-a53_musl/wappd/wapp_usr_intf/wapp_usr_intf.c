...
#endif
#ifdef MAP_R3_WF6
		{WAPP_USER_GET_ASSOC_WIFI6_STA_STATUS,         WAPP_ASSOC_WIFI6_STA_STATUS,       ASYNC,  default_match_func},
		{WAPP_USER_GET_DEV_INVEN_TLV,         WAPP_DEV_INVEN_TLV,       SYNC,  default_match_func},
		{WAPP_USER_GET_AP_WF6_CAPABILITY,              WAPP_AP_WF6_CAPABILITY,                          SYNC,   NULL},
#endif
#ifdef MAP_R3_RECONFIG
		{WAPP_USER_GET_RECONFIG_TRIGGER,              WAPP_RECONFIG_STATUS,     SYNC,   default_match_func},
#endif
};
	...
	socklen_t fromlen, char* daemon_name)
{
	...
	ret = os_snprintf(dst->daemon_name, sizeof(dst->daemon_name), "%s", daemon_name);
	if (os_snprintf_error(sizeof(dst->daemon_name), ret)) {
					...
					socklen_t fromlen, char *buf, size_t buf_len, char** reply, size_t *resp_len)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, "%s\n", __func__);
	reply_buf = (char*)os_malloc(WAPP_MAP_REPLY_BUFFER);
	if (reply_buf == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, "%s alloc relay_buf fail!\n", __func__);
		goto error3;
	}
	os_memset(reply_buf, 0, WAPP_MAP_REPLY_BUFFER);
...
int retry_socket_send(int sock){
	...
	if(sock < 0) {
		DBGPRINT(RT_DEBUG_OFF, "  retry_socket_send sock:%d invalid \n", sock);
		return 0;
	}
...
void wapp_iface_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	memset(wapp->map_wapp_buffer, 0, wapp->map_wapp_buffer_size);
	res = recvfrom(sock, wapp->map_wapp_buffer, wapp->map_wapp_buffer_size - 1, 0, (struct sockaddr *) &from, &fromlen);
	if (res < 0) {
		DBGPRINT(RT_DEBUG_ERROR, "recvfrom(ctrl_iface): %s", strerror(errno));
		return;
	}
	if ((res > (wapp->map_wapp_buffer_size - 1)) || res == 0) {
		DBGPRINT(RT_DEBUG_ERROR, "recvfrom(ctrl_iface):buffer size is less res = %d", res);
		return;
	}
	wapp->map_wapp_buffer[res] = 0;
	/*
	hex_dump_dbg("wapp recv cmd:", (unsigned char*)buf, res);
	*/
	if (os_strncmp(wapp->map_wapp_buffer, "ATTACH:", strlen("ATTACH:")) == 0) {
		daemon_name = strchr(wapp->map_wapp_buffer, ':');
		if (daemon_name != NULL) {
			daemon_name++;
	...
	} else if (os_strncmp(wapp->map_wapp_buffer, "DETACH", strlen("DETACH")) == 0) {
		...
		wapp_iface_process(wapp, &from, fromlen, wapp->map_wapp_buffer, res, &reply_buf, &reply_len);
		reply = reply_buf;
	}
...
int wapp_iface_open_sock(struct wifi_app *wapp, char* socket_patch)
{
	...
	ret = os_snprintf(cli_ctrl->addr.sun_path, sizeof(cli_ctrl->addr.sun_path), "%s", buf);
	if (os_snprintf_error(sizeof(cli_ctrl->addr.sun_path), ret)) {
		DBGPRINT(RT_DEBUG_ERROR, "[%s] os_snprintf fail\n", __func__);
		goto fail;
	}
