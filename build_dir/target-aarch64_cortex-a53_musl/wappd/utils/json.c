...
static struct json_token * json_alloc_token(unsigned int *tokens)
{
	...
	if (*tokens > JSON_MAX_TOKENS) {
		wpa_printf(MSG_DEBUG, DPP_MAP_PREX "JSON: Maximum token limit exceeded");
		return NULL;
	}
...
struct json_token * json_parse(const char *data, size_t data_len)
{
	...
	for (; pos < end; pos++) {
		...
		case '\"': /* string */
			...
			if (!curr_token) {
				...
				wpa_printf(MSG_MSGDUMP, DPP_MAP_PREX
					   "JSON: String value: '%s'",
					   curr_token->string);
	...
	if (json_check_tree_state(root) < 0) {
		wpa_printf(MSG_DEBUG, DPP_MAP_PREX "JSON: Incomplete token in the tree");
		goto fail;
	}

	return root;
fail:
	wpa_printf(MSG_DEBUG, DPP_MAP_PREX "JSON: Parsing failed");
	json_free(root);
	if (str)
		os_free(str);
	return NULL;
					  ...
					  const char *name)
{
	...
	if (!token || token->type != JSON_STRING)
		return NULL;
	buf = base64_url_decode(token->string,
		       ...
		       size_t len)
{
	...
	b64 = (char *)base64_url_encode(val, len, NULL, 0);
	if (!b64)
		return -1;
