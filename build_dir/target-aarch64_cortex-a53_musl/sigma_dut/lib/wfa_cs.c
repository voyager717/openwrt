...
extern int IsDatCopied;
extern int IsScanDone;
...
#else
	sprintf(gCmdStr, "iwpriv %s set ApCliCertAggLimit=1", intf);
#endif
    sret = system(gCmdStr);
...
//don't need to set ApCliAutoConnect again, this will lead connect twice!
		/* ApCliAutoConnect required for TC SAE-5.7.3 */
		if (!IsScanDone) {
			sleep(2);
	   ...
	   DPRINT_INFO(WFA_OUT, "%s() inf=%s type=%u keyMgmtType=%s encpType=%s\n",
		...
		)
		{
		   ...
		   else if((set_sec->type == SEC_TYPE_SAE) && (strcasecmp(set_sec->keyMgmtType, "wpa2") == 0) && (strcasecmp(set_sec->encpType, "aes-ccmp") == 0))
		   {
				...
				if (set_sec->sae_pk != 0) {
					wfa_driver_mtk_set_cmd(cmd->intf, "ApCliSAEPK=1");
				}
				if (set_sec->sae_pk_only != 0) {
					wfa_driver_mtk_set_cmd(cmd->intf, "ApCliSAEPKOnly=1");
				}
		   ...
		   else if((set_sec->type == SEC_TYPE_PSKSAE) && (strcasecmp(set_sec->keyMgmtType, "wpa2-psk-sae") == 0) && (strcasecmp(set_sec->encpType, "aes-ccmp") == 0))
		   {
				wfa_driver_mtk_set_cmd(cmd->intf, "ApCliAuthMode=WPA2PSKWPA3PSK");
				wfa_driver_mtk_set_cmd(cmd->intf, "ApCliEncrypType=AES");
...
#endif /* DRIVER_SCAN_TRIGGER */
				wfa_driver_mtk_set_cmd(cmd->intf, "CertCfg=1");
		}
		else
		{
...
#ifdef MASK_PARTIAL_MACADDR
   DPRINT_INFO(WFA_OUT, "wfaStaGetBSSID = "MACSTR"\n", MAC2STR(bssidResp->cmdru.bssid));
#else
...
int wfaStaPresetParams(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    DPRINT_INFO(WFA_OUT, "%s:%d IsDatCopied=%d\n", __func__, __LINE__, IsDatCopied);

    if (!IsDatCopied) {
	    wfa_driver_mtk_reset_profile();
	    sprintf(gCmdStr, "/sbin/wifi down; ifconfig %s up;", ifname);
	    sret = system(gCmdStr);
    }
...
int wfaStaSetWireless(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
	...
	/* set 40 BW for case OCV-15.2 */
	if(sw->bw == BW40)
	{
		wfa_driver_mtk_set_cmd(intf, "VhtBw=0");
	}
	// program,HE
...
int wfaStaResetDefault(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
	...
	wMEMCPY(prog_name, reset->prog,strlen(reset->prog));
	...
	system("sed -i 's/MboSupport=0/MboSupport=1/g' /etc/wireless/sigma_test/wifi_cert_b2.dat");
	system("sed -i 's/RRMEnable=0/RRMEnable=1/g' /etc/wireless/sigma_test/wifi_cert_b2.dat");
#endif
    ...
    /*
     * For TCac certification, the AMSDU_NUM should be changed
     * to avoid TestBed sniffer Crash
     */
    if (strcasecmp(reset->prog, "VHT") == 0) {
	DPRINT_INFO(WFA_OUT, "AMSDU_NUM changed to 2 for TGac cert\n");
	system("sed -i 's/AMSDU_NUM=8/AMSDU_NUM=2/g' /etc/wireless/sigma_test/wifi_cert_b0.dat");
	system("sed -i 's/AMSDU_NUM=8/AMSDU_NUM=2/g' /etc/wireless/sigma_test/wifi_cert_b1.dat");
#ifdef FIRST_CARD_MT7986_AX8400
	system("sed -i 's/AMSDU_NUM=8/AMSDU_NUM=2/g' /etc/wireless/sigma_test/wifi_cert_b2.dat");
#endif /* FIRST_CARD_MT7986_AX8400 */
    }

    if (strcasecmp(reset->prog, "PMF") || strcasecmp(reset->prog, "WPA3") == 0)
	wfa_driver_mtk_set_cmd(reset->intf, "Debug=1");

    wfa_driver_mtk_reset_profile();
    sprintf(gCmdStr, "/sbin/wifi down; ifconfig %s up;", reset->intf);
    sret = system(gCmdStr);
    if (strcasecmp(reset->prog, "MBO") == 0) {
		sprintf(gCmdStr, "ifconfig ra%s up", reset->intf + 5);
		sret = system(gCmdStr);
	    sprintf(gCmdStr, "wapp_openwrt.sh");
	    sret = system(gCmdStr);
	    sleep(3);
	...
	if((strncmp(reset->intf,"apcli", 5) == 0)
		...
		if(strcasecmp(reset->prog, "wpa3") == 0) {
			...
			wfa_driver_mtk_set_cmd(reset->intf, "ApCliTransDisableSupported=1");
			wfa_driver_mtk_set_cmd(reset->intf, "ApCliSAEPKOnly=0");
...
int wfaStaDevSendFrame(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    case PROG_TYPE_WPA3:
    {
        wpa3Frame_t *wpa3 = &sf->frameType.wpa3;
        switch (wpa3->eFrameName)
        {
        case WPA3_FRAME_BEACON:
            break;
        case WPA3_FRAME_EAPOL_M3:
            break;
        case WPA3_FRAME_SA_QUERY_REQ:
            break;
        case WPA3_FRAME_CHANNEL_SWITCH_ANNOUNCEMENT:
            break;
        case WPA3_FRAME_REASSOC_REQ:
            break;
        case WPA3_FRAME_ANQP_QUERY:
            if (strlen(wpa3->sDestMac) != WFA_MAC_ADDR_STR_LEN - 1) {
                DPRINT_ERR(WFA_ERR, "invalid DestMac: %s\n", wpa3->sDestMac);
                break;
            }

            if (strlen(wpa3->sInterface) == 0) {
                DPRINT_ERR(WFA_ERR, "invalid Interface\n");
                break;
            }

            if (wpa3->ChnlFreq <= 0) {
                DPRINT_ERR(WFA_ERR, "invalid ChnlFreq\n");
                break;
            }

            sprintf(gCmdStr, "iwpriv %s set Anqp=1,%d,%s", wpa3->sInterface, wpa3->ChnlFreq, wpa3->sDestMac);
            DPRINT_INFO(WFA_OUT, "cmd = %s\n", gCmdStr);
            system(gCmdStr);
            break;
        }
