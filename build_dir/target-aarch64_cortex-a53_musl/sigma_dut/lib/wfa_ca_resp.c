...
int wfaStaGetMacAddressResp(BYTE *cmdBuf)
{
    ...
    case STATUS_COMPLETE:
        ...
        printf("status,COMPLETE,mac,"MACSTR"\r\n", MAC2STR(getmacResp->cmdru.mac));
#else
...
#ifdef MASK_PARTIAL_MACADDR
        printf("status,COMPLETE,bssid,"MACSTR"\r\n", MAC2STR(getBssidResp->cmdru.bssid));
#else
...
#ifdef MASK_PARTIAL_MACADDR
        printf("status,COMPLETE,bssid,"MACSTR"\r\n", MAC2STR(execActionResp->cmdru.execAction.mac));
#else
        printf("status,COMPLETE,mac,%s\r\n", execActionResp->cmdru.execAction.mac);
#endif
        break;
...
#ifdef MASK_PARTIAL_MACADDR
        printf("status,COMPLETE,EventName,%s,RemoteInstanceID,%u,LocalInstanceID,%u,mac,"MACSTR"\r\n",
            getEvents->eventName,getEvents->remoteInstanceID,getEvents->localInstanceID,
            MAC2STR(getEvents->mac));
#else
