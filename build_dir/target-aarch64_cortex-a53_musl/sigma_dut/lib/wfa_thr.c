    ...
    while(1)
    {
        ...
        case DIRECT_SEND:
            ...
            if (myProfile->profile == PROF_IPTV || myProfile->profile == PROF_FILE_TX || myProfile->profile == PROF_MCAST)
            {
                int iOptVal, iOptLen;
		int err;
                err = getsockopt(mySock, SOL_SOCKET, SO_SNDBUF, (char *)&iOptVal, (socklen_t *)&iOptLen);
		if(err<0) {
			printf("getsockopt error, %d\n",errno);
		}
		printf("getter of scoket size iOptVal= %d\n",iOptVal);
                iOptVal = iOptVal * 16;
                err = setsockopt(mySock, SOL_SOCKET, SO_SNDBUF, (char *)&iOptVal, (socklen_t )iOptLen);
		if(err<0) {
			printf("setsockopt error, %d\n",errno);
		}
		printf("setter of scoket size iOptVal= %d\n",iOptVal);
