...
VOID token_rx_dmad_pool_dump(struct token_rx_pkt_queue *que)
{
	...
	for (token_loop = 0 ; token_loop < que->pkt_tkid_cnt ; token_loop++) {
		MTWF_DBG(NULL, DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			"token[%04u]: PA=%08x, VA=%08x, pkt=%08x \n",
			token_loop, (que->pkt_token[token_loop].dma_buf.AllocPa), (que->pkt_token[token_loop].dma_buf.AllocVa), (que->pkt_token[token_loop].pkt_buf));
	}
	return;
}
