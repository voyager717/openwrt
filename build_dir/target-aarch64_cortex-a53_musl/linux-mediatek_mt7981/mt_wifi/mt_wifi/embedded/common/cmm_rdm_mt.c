...
#endif
	MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_INFO, "DFS ch %d is selected, use non-DFS ch %d, ch_stat %d\n",
	...
	)
{
	...
	if (IS_CH_BETWEEN(*ch, 36, 64)) {
			...
			} else if ((vht_bw == VHT_BW_160) && IS_CH_BETWEEN(*ch, 36, 64) && (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState != DFS_BW160_TX160RX160)) {
				pAd->CommonCfg.DfsParameter.BW160ZeroWaitState = DFS_BW160_TX80RX160;
				pAd->CommonCfg.DfsParameter.OutBandCh = *ch;
				*ch = 36;
				vht_bw =  VHT_BW_80;
		 ...
		 */
			...
			if (vht_bw == VHT_BW_160)
				DfsAdjustBwSetting(pAd, wdev, VHT_BW_160, VHT_BW_80);
		}
		...
		for (BssIdx = 0; BssIdx < pAd->ApCfg.BssidNum; BssIdx++) {
			struct wifi_dev *wdev = &pAd->ApCfg.MBSSID[BssIdx].wdev;
	...
	MLME_QUEUE_ELEM *Elem)
{
	...
	for (BssIdx = 0; BssIdx < pAd->ApCfg.BssidNum; BssIdx++) {
		...
		if (IS_ADJ_BW_ZERO_WAIT_TX80RX160(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState)
			|| (ZwUpdateCount != 0 && ZwUpdateCount < pAd->ApCfg.BssidNum)
			...
			if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState == DFS_BW160_TX80RX160 && ZwStatusUpdate == FALSE) {
				MTWF_DBG(pAd, DBG_CAT_CHN, CATCHN_DFS, DBG_LVL_WARN, "[RDM] APCLI UPDATE BW.\n");
			...
			|| (ZwUpdateCount != 0 && ZwUpdateCount < pAd->ApCfg.BssidNum)
			&& IS_ADJ_BW_ZERO_WAIT_BW80(pAd->CommonCfg.DfsParameter.BW160ZeroWaitState))
			DfsAdjustBwSetting(pAd, wdev, BW_80, BW_160);

			ap_chnl_switch_xmit(pAd, wdev, new_ch, BW_160);
			pDot11h->RDMode = RD_NORMAL_MODE;

			if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState == DFS_BW80_TX80RX160 && ZwStatusUpdate == FALSE)
				pAd->CommonCfg.DfsParameter.BW160ZeroWaitState = DFS_BW80_TX80RX80;
			else if (pAd->CommonCfg.DfsParameter.BW160ZeroWaitState == DFS_BW160_TX80RX160 && ZwStatusUpdate == FALSE)
