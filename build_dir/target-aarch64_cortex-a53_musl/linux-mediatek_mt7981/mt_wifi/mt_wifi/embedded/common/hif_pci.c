	...
	PNDIS_PACKET cp_skb_pkt)
{
	...
	for (LoopCnt = 0; LoopCnt < (scatterCnt - 1); LoopCnt++) {
		...
		/* Only gather the non PPE and non RRO Rx  (and mark as white list by WO) */
		if ((!(*dw3 & RXDMAD_PPE_VLD))
			&& (!(*dw1 & RXDMAD_TO_HOST_A))
#ifdef WO_SCATTER_INDICATE
			&& (*dw2 & RXDMAD_WO_INDICATE_SCATTER)
		...
		NdisMoveMemory((UCHAR *)&(pAd->RxDHisLog[pAd->rxd_log_idx]), (UCHAR *)pCurRxD, RXD_SIZE);
		/* NdisMoveMemory((UCHAR *)&(pAd->RxDHisLog[pAd->rxd_log_idx].buffer), (UCHAR *)cur_alloc_va, 20); */
	...
	UCHAR resource_idx)
{
	...
	scatterCnt = rx_scatter_info_wed(rx_ring, pRxD, &gather_size);
#else /* RXD_WED_SCATTER_SUPPORT */
	scatterCnt = rx_scatter_info(rx_ring, pRxD, &gather_size);
#endif /* !RXD_WED_SCATTER_SUPPORT */

	build_skb_len = SKB_DATA_ALIGN(SKB_BUF_HEADROOM_RSV + gather_size) +
				SKB_DATA_ALIGN(SKB_BUF_TAILROOM_RSV);
			...
			*/
			...
			if (scatterCnt > 1) {
				if (rx_scatter_gather_copy(pAd, resource_idx, pRxCell,
					pRxD, scatterCnt, cp_skb_pkt) == FALSE)	{
#if defined(CONFIG_WIFI_PAGE_ALLOC_SKB)
#if (KERNEL_VERSION(4, 10, 0) <= LINUX_VERSION_CODE)
			...
			/*build skb*/
#if defined(CONFIG_WIFI_PAGE_ALLOC_SKB)
			build_rx_pkt_skb(&skb_pkt, (VOID *)cp_skb_pkt, build_skb_len, gather_size);
#else
			skb_pkt = cp_skb_pkt;
#endif
