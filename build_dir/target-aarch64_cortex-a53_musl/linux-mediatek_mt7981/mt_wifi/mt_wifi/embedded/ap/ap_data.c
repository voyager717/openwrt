...
#if defined(SOFT_ENCRYPT) || defined(ADHOC_WPA2PSK_SUPPORT) || defined(WTBL_TDD_SUPPORT)
	NDIS_STATUS status;
	...
	if (pInactEntry ||
		...
		if ((pFmeCtrl->Wep == 1) && (pRxInfo->Decrypted == 0)) {
			...
			if (IS_CIPHER_CCMP128(pEntry->SecConfig.PairwiseCipher)) {
				...
				os_move_mem(PairwiseKey.Key, &pEntry->SecConfig.PTK[LEN_PTK_KCK + LEN_PTK_KEK], LEN_TK);
