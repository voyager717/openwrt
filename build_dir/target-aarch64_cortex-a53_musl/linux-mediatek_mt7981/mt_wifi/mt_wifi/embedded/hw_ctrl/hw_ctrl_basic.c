...
static struct mtk_notifier_block {
	struct notifier_block nb;
	struct mutex lock;
	void *priv;
} mtk_fe_reset_notifier;
...
static int mtk_fe_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	case MTK_FE_START_RESET:
		...
		pErrRecoveryCtrl->notify_fe = TRUE;
#ifdef WIFI_UNIFIED_COMMAND
...
unsigned int mtk_fe_reset_notifier_init(struct _RTMP_ADAPTER  *pAd)
{
	...
	mtk_fe_reset_notifier.priv = pAd;
	mtk_fe_reset_notifier.nb.notifier_call = mtk_fe_event;
	mutex_init(&mtk_fe_reset_notifier.lock);
	init_completion(&pErrRecoveryCtrl->fe_reset_done);

	err = register_netdevice_notifier(&mtk_fe_reset_notifier.nb);
	if (err)
		mutex_destroy(&mtk_fe_reset_notifier.lock);
...
{
	mutex_destroy(&mtk_fe_reset_notifier.lock);
	unregister_netdevice_notifier(&mtk_fe_reset_notifier.nb);
}
#endif
