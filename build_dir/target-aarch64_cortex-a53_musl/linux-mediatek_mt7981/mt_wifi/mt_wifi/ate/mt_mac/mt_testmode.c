...
INT32 MT_ATEDumpLog(RTMP_ADAPTER *pAd, struct _ATE_LOG_DUMP_CB *log_cb, UINT32 log_type)
{
	...
	case ATE_LOG_RXV:
		...
		ret = snprintf(Log_type, sizeof(Log_type), "%s", "LOG");
		if (os_snprintf_error(sizeof(Log_type), ret)) {
	...
	case ATE_LOG_RDD:
		...
		ret = snprintf(Log_type, sizeof(Log_type), "%s", "RDD");
		if (os_snprintf_error(sizeof(Log_type), ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"Log_type snprintf error!\n");
			return FALSE;
	...
	case ATE_LOG_RE_CAL:
		...
		ret = snprintf(Log_type, sizeof(Log_type), "%s", "RECAL");
		if (os_snprintf_error(sizeof(Log_type), ret)) {
			MTWF_DBG(pAd, DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				"Log_type snprintf error!\n");
			return FALSE;
...
INT MtATE_DPD_Cal_Store_Proc_7916(RTMP_ADAPTER *pAd, UINT8 op)
{
    ...
    if (op == PREK_DPD_CLEAN) {
		...
		/* If want to debug, we can use only ch36 to verify and need pay attention to the index */
		for (i = 0; i < MT7916_DPD_FLATNESS_ABAND_BW20_CH_SIZE; i++) {
			/* set channel command , per group calibration - set to channel 36, 52, BW20 */
			ch_cfg.Bw = BW_20;
			ch_cfg.CentralChannel = MT7916_DPD_FLATNESS_ABAND_BW20_CH[i];
			ch_cfg.ControlChannel = MT7916_DPD_FLATNESS_ABAND_BW20_CH[i];
			ch_cfg.ControlChannel2 = 0;
			ch_cfg.BandIdx = 1;
			ch_cfg.bScan = 0;
			ch_cfg.Channel_Band = 1;
		...
		/* If want to debug, we can use only ch6 to verify and need pay attention to the index */
		for (i = 0; i < MT7916_DPD_FLATNESS_GBAND_BW20_CH_SIZE; i++) {
			/* set channel command , per group calibration - set to channel 1, 6, 11, BW20 */
			ch_cfg.Bw = BW_20;
			ch_cfg.CentralChannel = MT7916_DPD_FLATNESS_GBAND_BW20_CH[i];
			ch_cfg.ControlChannel = MT7916_DPD_FLATNESS_GBAND_BW20_CH[i];
			ch_cfg.ControlChannel2 = 0;
			ch_cfg.BandIdx = 0;
			ch_cfg.bScan = 0;
			ch_cfg.Channel_Band = 0;
