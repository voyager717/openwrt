...
return baseclass.extend({
	...
	handleMenuExpand: function(ev) {
		...
		/*
		 * 2023-10-23: zzk
		 * 发生事件的ul1标签和子标签class属性全部加上'active'
		 * 导致已经下拉的菜单无法收回
		 * 标签a是ul1的子标签，即：ul1.classList.remove('active')
		...
		/*
		 * 解决已经下拉的菜单无法收回的问题
		 * 判断该标签class属性是否有'active'，有则移除
		 * 当再次点击菜单的时候移除'active'收回菜单
		 */
		if(ul1.classList.contains('active')){

			console.log('ul1 Have active.');
			ul1.classList.remove('active');
			//与a.classList.remove('active');效果相同
			ul1.childNodes[0].classList.remove('active');
		}		
		else{
			console.log('ul1 No have active.');
			ul1.classList.add('active');
			//与a.classList.add('active');效果相同
			ul1.childNodes[0].classList.add('active');
		}
		//2023-10-23: zzk end
	...
	renderMainMenu: function(tree, url, level) {
		...
		for (var i = 0; i < children.length; i++) {
			...
			//保持菜单状态，判断菜单li标签的class原本有无‘'active'，若有则加上'active'下拉展开，否则为空收回
