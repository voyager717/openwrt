...
static FILE *fpopen(const char *dir, const char *name)
{
	...
	ret = snprintf(path, PATH_MAX, "%s/%s", dir, name);
	if (ret < 0 || ret >= PATH_MAX)
		dprintf("[%d]snprintf fail!\n", __LINE__);
...
static void fetch_id(const char *dev, const char *name, struct bridge_id *id)
{
	...
	if (!f) {
		if (fprintf(stderr, "%s: %s\n", dev, strerror(errno)) < 0)
			dprintf("[%d]fprintf fail!\n", __LINE__);
	} else {
		if (fscanf(f, "%2hhx%2hhx.%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx",
				&id->prio[0], &id->prio[1],
				&id->addr[0], &id->addr[1], &id->addr[2],
				&id->addr[3], &id->addr[4], &id->addr[5]) < 0)
			dprintf("[%d]fscanf fail!\n", __LINE__);
		if (fclose(f) < 0)
			dprintf("[%d]fclose fail!\n", __LINE__);
	}
}
...
int br_get_bridge_info(const char *bridge, struct bridge_info *info)
{
	...
	ret = snprintf(path, SYSFS_PATH_MAX, SYSFS_CLASS_NET "%s/bridge", bridge);
		     ...
		     struct port_info *info)
{
	...
	ret = snprintf(path, SYSFS_PATH_MAX, SYSFS_CLASS_NET "%s/brport", port);
	if (ret < 0 || ret >= SYSFS_PATH_MAX) {
		dprintf("[%d]snprintf fail!\n", __LINE__);
		goto fallback;
	}
		  ...
		  unsigned long value, unsigned long oldcode)
{
	...
	ret = snprintf(path, SYSFS_PATH_MAX, SYSFS_CLASS_NET "%s/bridge/%s",
				   bridge, name);
	if (ret < 0 || ret >= SYSFS_PATH_MAX)
		dprintf("[%d]snprintf fail!\n", __LINE__);
		    ...
		    unsigned long oldcode)
{
	...
	ret = snprintf(path, SYSFS_PATH_MAX, SYSFS_CLASS_NET "%s/brport/%s", ifname, name);
	if (ret < 0 || ret >= SYSFS_PATH_MAX)
		dprintf("[%d]snprintf fail!\n", __LINE__);
		...
		unsigned long offset, int num)
{
	...
	ret = snprintf(path, SYSFS_PATH_MAX, SYSFS_CLASS_NET "%s/brforward", bridge);
	if (ret < 0 || ret >= SYSFS_PATH_MAX)
		dprintf("[%d]snprintf fail!\n", __LINE__);
	f = fopen(path, "r");
	if (f) {
		if (fseek(f, offset*sizeof(struct __fdb_entry), SEEK_SET) != 0)
			dprintf("[%d]fseek fail!\n", __LINE__);
		n = fread(fe, sizeof(struct __fdb_entry), num, f);
		if (n != num)
			dprintf("[%d]fread fail!\n", __LINE__);
		if (fclose(f) < 0)
			dprintf("[%d]fclose fail!\n", __LINE__);
	} else {
