...
int dpp_check_attrs(const u8 *buf, size_t len)
{
	...
	while (end - pos >= 4) {
		...
		alen = WPA_GET_LE16(pos);
		pos += 2;
		debug(DEBUG_TRACE, "DPP: Attribute ID %04x len %u\n",
			   id, alen);
		if (alen > end - pos) {
			debug(DEBUG_ERROR,
		...
		size_t attr_len)
{
	...
	default:
		...
		debug(DEBUG_ERROR, "Invalid role in I-capabilities 0x%02x",
					   ...
					   enum dpp_status_error status)
{
	...
	if (dpp_test == DPP_TEST_AFTER_WRAPPED_DATA_AUTH_CONF)
		attr_len += 5;
#endif /* CONFIG_TESTING_OPTIONS */
	msg = dpp_alloc_msg(DPP_PA_AUTHENTICATION_CONFIRM, attr_len);
	if (!msg)
		goto fail;
...
int dpp_akm_dpp(enum dpp_akm akm)
{
	return akm == DPP_AKM_DPP || akm == DPP_AKM_SAE_DPP ||
		akm == DPP_AKM_PSK_SAE_DPP || akm == DPP_AKM_PSK_DPP;
}


int dpp_akm_ver2(enum dpp_akm akm)
{
	return akm == DPP_AKM_SAE_DPP || akm == DPP_AKM_PSK_SAE_DPP || akm == DPP_AKM_PSK_DPP;
}
...
int dpp_parse_cred_legacy(struct dpp_authentication *auth,
...
static int dpp_check_pubkey_match(EVP_PKEY *pub, struct wpabuf *r_hash)
{
	...
	if (!uncomp) {
		debug(DEBUG_ERROR,
			   "DPP: dpp_get_pubkey_point fail\n");
		return -1;
    }
	addr[0] = wpabuf_head(uncomp);
	len[0] = wpabuf_len(uncomp);
	hex_dump_info("DPP: Uncompressed public key",
		    (unsigned char *)addr[0], (unsigned int)len[0]);
	...
	if (os_memcmp(hash, wpabuf_head(r_hash), SHA256_MAC_LEN) != 0) {
		...
		hex_dump_info("DPP: Calculated hash",
			    hash, SHA256_MAC_LEN);
		return -1;
	}
	return 0;
			     ...
			     EVP_PKEY *csign_pub, const char *connector)
{
	...
	if (!curve)
		goto fail;
	debug(DEBUG_TRACE, "DPP: C-sign-key group: %s\n", curve->jwk_crv);
	os_memset(info, 0, sizeof(*info));

	signed_start = pos = connector;
	end = os_strchr(pos, '.');
	if (!end) {
		debug(DEBUG_ERROR, "DPP: Missing dot(1) in signedConnector\n");
	...
	hex_dump("DPP: signedConnector - JWS Protected Header",
	...
	hex_dump("DPP: signedConnector - JWS Payload",
	...
	hex_dump("DPP: signedConnector - signature",
		    signature, signature_len);

	if (dpp_check_pubkey_match(csign_pub, kid) < 0) {
		debug(DEBUG_ERROR,
			   "DPP: pubkey not match\n");
	...
	if (signature_len & 0x01) {
		debug(DEBUG_ERROR,
			   "DPP: Unexpected signedConnector signature length (%d)n",
			   (int) signature_len);
		ret = DPP_STATUS_INVALID_CONNECTOR;
		goto fail;
	}
	...
	if (der_len <= 0) {
		debug(DEBUG_ERROR, "DPP: Could not DER encode signature\n");
		goto fail;
	}
	hex_dump("DPP: DER encoded signature", der, der_len);
	md_ctx = EVP_MD_CTX_create();
	if (!md_ctx)
		goto fail;
	...
	if (EVP_DigestVerifyInit(md_ctx, NULL, sign_md, NULL, csign_pub) != 1) {
		debug(DEBUG_ERROR, "DPP: EVP_DigestVerifyInit failed: %s\n",
			   ERR_error_string(ERR_get_error(), NULL));
		goto fail;
	}
	if (EVP_DigestVerifyUpdate(md_ctx, signed_start,
				   signed_end - signed_start + 1) != 1) {
		debug(DEBUG_ERROR, "DPP: EVP_DigestVerifyUpdate failed: %s\n",
			   ERR_error_string(ERR_get_error(), NULL));
		goto fail;
	}
...
{
#ifdef MAP_R3
        return (os_strcmp(role1, "mapAgent") == 0 && os_strcmp(role2, "mapController") == 0) ||
	       ...
	       os_time_t *expiry)
{
	...
	if (!csign) {
		debug(DEBUG_ERROR,
			   "[dpp deer discovery]: Failed to parse local C-sign-key information\n");
		goto fail;
	}
	...
	own_key = dpp_set_keypair(&own_curve, net_access_key,
				  net_access_key_len);
	if (!own_key) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: Failed to parse own netAccessKey\n");
		goto fail;
	}

	pos = os_strchr(own_connector, '.');
	if (!pos) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: Own connector is missing the first dot (.)\n");
		goto fail;
	}
	pos++;
	end = os_strchr(pos, '.');
	if (!end) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: Own connector is missing the second dot (.)\n");
		goto fail;
	}
	...
	if (!own_conn) {
		debug(DEBUG_ERROR,
			   "[dpp deer discovery]: Failed to base64url decode own signedConnector JWS Payload\n");
		goto fail;
	}

	own_root = json_parse((const char *) own_conn, own_conn_len);
	if (!own_root) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: Failed to parse local connector\n");
		goto fail;
	}

	wpa_hexdump_ascii(MSG_DEBUG, "[dpp deer discovery]: Peer signedConnector",
	...
	if (res != DPP_STATUS_OK) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: process signed connector fail\n");
		ret = res;
		goto fail;
	}
	...
	if (!root) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: JSON parsing of connector failed\n");
		ret = DPP_STATUS_INVALID_CONNECTOR;
		goto fail;
	}
	...
	if (!dpp_connector_match_groups(own_root, root)) {
		debug(DEBUG_ERROR,
			   "[dpp deer discovery]: Peer connector does not include compatible group netrole with own connector\n");
		ret = DPP_STATUS_NO_MATCH;
		goto fail;
	}
	...
	if (!token || token->type != JSON_STRING) {
		debug(DEBUG_ERROR,
			   "[dpp deer discovery]: No expiry string found - connector does not expire\n");
	} else {
		debug(DEBUG_ERROR, "DPP: expiry = %s\n", token->string);
		if (dpp_key_expired(token->string, expiry)) {
			debug(DEBUG_ERROR,
				   "[dpp deer discovery]: Connector (netAccessKey) has expired\n");
			ret = DPP_STATUS_INVALID_CONNECTOR;
			goto fail;
		}
	}
	...
	netkey = json_get_member(root, "netAccessKey");
	if (!netkey || netkey->type != JSON_OBJECT) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: No netAccessKey object found\n");
		ret = DPP_STATUS_INVALID_CONNECTOR;
		goto fail;
	}
	...
	if (!peer_key) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: dpp parse peer key fail\n");
		ret = DPP_STATUS_INVALID_CONNECTOR;
		goto fail;
	}
	dpp_debug_print_key("[dpp deer discovery]: Received netAccessKey", peer_key);

	if (own_curve != curve) {
		debug(DEBUG_ERROR,
			   "[dpp deer discovery]: Mismatching netAccessKey curves (%s != %s)\n",
	...
	if (dpp_ecdh(own_key, peer_key, Nx, &Nx_len) < 0) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: dpp_ecdh fail\n");
		goto fail;
    }

	hex_dump("[dpp deer discovery]: ECDH shared secret (N.x)",
			Nx, Nx_len);

	/* PMK = HKDF(<>, "DPP PMK", N.x) */
	if (dpp_derive_pmk(Nx, Nx_len, intro->pmk, curve->hash_len) < 0) {
		debug(DEBUG_ERROR, "DPP: Failed to derive PMK\n");
		goto fail;
	}
	...
	if (dpp_derive_pmkid(curve, own_key, peer_key, intro->pmkid) < 0) {
		debug(DEBUG_ERROR, "[dpp deer discovery]: Failed to derive PMKID\n");
		goto fail;
	}
...
int dpp_bootstrap_gen(struct dpp_global *dpp, const char *cmd)
{
	...
	if (!bi->uri)
		goto fail;
	res = os_snprintf(bi->uri, len + 1, "DPP:%s%s%s%s%s%s%s%s%sK:%s;;",
		    chan ? "C:" : "", chan ? chan : "", chan ? ";" : "",
		    mac ? "M:" : "", mac ? mac : "", mac ? ";" : "",
		    info ? "I:" : "", info ? info : "", info ? ";" : "",
		    pk);
	if (os_snprintf_error(len + 1, res)) {
					...
					int initiator)
{
	...
	debug(DEBUG_ERROR, "init=%d", initiator);
#ifdef CONFIG_TESTING_OPTIONS
	if (dpp_test == DPP_TEST_STOP_AT_AUTH_CONF) {
		wpa_printf(MSG_INFO,
			   "DPP: TESTING - stop at Authentication Confirm");
