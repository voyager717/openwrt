...
int eloop_init(void)
{
	...
	if (eloop.epollfd < 0) {
		debug(MSG_ERROR, "%s: epoll_create1 failed. %s\n",
                                     ...
                                     void *eloop_data, void *user_data)
{
	...
	if (eloop.count + 1 > eloop.epoll_max_event_num) {
		...
		if (temp_events == NULL) {
			debug(MSG_ERROR, "%s: malloc for epoll failed. "
				   "%s\n", __func__, strerror(errno));
			return -1;
		}
...
static void eloop_sock_table_destroy(struct eloop_sock_table *table)
{
	if (table) {
		int i;
		for (i = 0; i < table->count && table->table; i++) {
			debug(MSG_INFO, "ELOOP: remaining socket: "
			   ...
			   void *eloop_data, void *user_data)
{
	...
	if (timeout->time.sec < now_sec) {
		 ...
		 */
		debug(MSG_DEBUG, "ELOOP: Too long timeout (secs=%u) to "
...
{
	/*debug(MSG_ERROR, "eloop: could not process SIGINT or SIGTERM in "
...
static void eloop_handle_signal(int sig)
{
	...
	if ((sig == SIGINT || sig == SIGTERM) && !eloop.pending_terminate) {
		...
		(void)signal(SIGALRM, eloop_handle_alarm);
		alarm(2);
	}
#endif /* CONFIG_NATIVE_WINDOWS */
			  ...
			  void *user_data)
{
	...
	if (signal(sig, eloop_handle_signal) == SIG_ERR)
		debug(DEBUG_ERROR, "[%d]signal fail\n", __LINE__);
...
void eloop_destroy(void)
{
	...
	dl_list_for_each_safe(timeout, prev, &eloop.timeout,
		...
		debug(MSG_INFO, "ELOOP: remaining timeout: %d.%06d "
