...
*/
    ...
    if (PlainBlockSize != AES_BLOCK_SIZES) {
    	debug(DEBUG_ERROR, "RT_AES_Encrypt: plain block size is %d bytes, it must be %d bytes(128 bits).\n",
            PlainBlockSize, AES_BLOCK_SIZES);
        return;
    }
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "RT_AES_Encrypt: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return;
    }
    if (*CipherBlockSize < AES_BLOCK_SIZES) {
    	debug(DEBUG_ERROR, "RT_AES_Encrypt: cipher block size is %d bytes, it must be %d bytes(128 bits).\n",
...
//	os_alloc_mem(NULL, (UCHAR **)&paes_ctx, sizeof(AES_CTX_STRUC));
	paes_ctx = (AES_CTX_STRUC *)malloc(sizeof(AES_CTX_STRUC));
	if (paes_ctx == NULL)
	{
		debug(DEBUG_ERROR, "Allocate memory fail!!!\n");
		return;
	}
	memset(paes_ctx, 0, sizeof(AES_CTX_STRUC));
    /*
     * 2. Transfer the plain block to state block
     */
...
*/
     ...
     */
    if (*PlainBlockSize < AES_BLOCK_SIZES) {
    	debug(DEBUG_ERROR, "RT_AES_Decrypt: plain block size is %d bytes, it must be %d bytes(128 bits).\n",
            *PlainBlockSize, AES_BLOCK_SIZES);
        return;
    }
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "RT_AES_Decrypt: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return;
    }
    if (CipherBlockSize != AES_BLOCK_SIZES) {
    	debug(DEBUG_ERROR, "RT_AES_Decrypt: cipher block size is %d bytes, it must be %d bytes(128 bits).\n",
            CipherBlockSize, AES_BLOCK_SIZES);
        return;
    }
    ...
    INOUT UINT *CipherTextLength)
{
    ...
    if (KeyLength != AES_KEY128_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CCM_Encrypt: The key length must be %d bytes\n", AES_KEY128_LENGTH);
        return -1;
    }

    if ((NonceLength < 7) || (NonceLength > 13)) {
    	debug(DEBUG_ERROR, "AES_CCM_Encrypt: A valid nonce length is 7-13 bytes\n");
        return -2;
    }

    if ((MACLength != 4) && (MACLength != 6) && (MACLength != 8) && (MACLength != 10)
        && (MACLength != 12) && (MACLength != 14) && (MACLength != 16)) {
    	debug(DEBUG_ERROR, "AES_CCM_Encrypt: The MAC length  must be 4, 6, 8, 10, 12, 14, or 16 bytes\n");
        return -3;
    }

    if (*CipherTextLength < (PlainTextLength + MACLength)) {
    	debug(DEBUG_ERROR, "AES_CCM_Encrypt: The CipherTextLength is not enough.\n");
        return -4;
    }
    ...
    INOUT UINT *PlainTextLength)
{
     ...
     */
    if (KeyLength != AES_KEY128_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CCM_Decrypt: The key length must be %d bytes\n", AES_KEY128_LENGTH);
        return -1;
    }

    if ((NonceLength < 7) || (NonceLength > 13)) {
    	debug(DEBUG_ERROR, "AES_CCM_Decrypt: A valid nonce length is 7-13 bytes\n");
        return -2;
    }

    if ((MACLength != 4) && (MACLength != 6) && (MACLength != 8) && (MACLength != 10)
        && (MACLength != 12) && (MACLength != 14) && (MACLength != 16)) {
    	debug(DEBUG_ERROR, "AES_CCM_Decrypt: The MAC length  must be 4, 6, 8, 10, 12, 14, or 16 bytes\n");
        return -3;
    }

    if (*PlainTextLength < (CipherTextLength - MACLength)) {
    	debug(DEBUG_ERROR, "AES_CCM_Decrypt: The PlainTextLength is not enough.\n");
        return -4;
    }
...
//    NdisZeroMemory(Block_CTR, AES_BLOCK_SIZES);
    ...
    for (Temp_Index = (CipherTextLength - MACLength); Temp_Index < CipherTextLength; Temp_Index++, Block_Index++) {
        if(Block_Index < AES_BLOCK_SIZES)
    ...
    while (Cipher_Index < (CipherTextLength - MACLength))
    {
        ...
        if (Copy_Length > AES_BLOCK_SIZES)
            Copy_Length = AES_BLOCK_SIZES;
        for (Temp_Index = 0; Temp_Index < Copy_Length; Temp_Index++) {
	     if(Cipher_Index + Temp_Index < *PlainTextLength)
...
//    if (NdisCmpMemory(Block_MAC_From_Cipher, Block_MAC, MACLength) != 0) {
    if (memcmp(Block_MAC_From_Cipher, Block_MAC, MACLength) != 0) {
    	debug(DEBUG_ERROR, "AES_CCM_Decrypt: The MIC does not match.\n");
        return -5;
    }
    ...
    OUT UINT8 SubKey2[])
{
    ...
    if (KeyLength != AES_KEY128_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CMAC_GenerateSubKey: key length is %d bytes, it must be %d bytes(128 bits).\n",
    ...
    INOUT UINT *MACTextLength)
{
    ...
    if (*MACTextLength < AES_MAC_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CMAC: MAC text length is less than %d bytes).\n",
            AES_MAC_LENGTH);
        return;
    }
    if (KeyLength != AES_KEY128_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CMAC: key length is %d bytes, it must be %d bytes(128 bits).\n",
            KeyLength, AES_KEY128_LENGTH);
        return;
    }
...
//    NdisMoveMemory(X, Const_Zero, AES_BLOCK_SIZES);
    memcpy(X, Const_Zero, AES_BLOCK_SIZES);
...
//        NdisMoveMemory(Y, &PlainText[PlainBlockStart], (PlainTextLength - PlainBlockStart));
        memcpy(Y, &PlainText[PlainBlockStart], (PlainTextLength - PlainBlockStart));
    ...
    INOUT UINT *CipherTextLength)
{
    ...
    if (*CipherTextLength < (PlainTextLength + PaddingSize)) {
    	debug(DEBUG_ERROR, "AES_CBC_Encrypt: cipher text length is %d bytes < (plain text length %d bytes + padding size %d bytes).\n",
            *CipherTextLength, PlainTextLength, PaddingSize);
        return;
    }
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "AES_CBC_Encrypt: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return;
    }
    if (IVLength != AES_CBC_IV_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CBC_Encrypt: IV length is %d bytes, it must be %d bytes(128bits).\n",
...
//    NdisMoveMemory(Block, (&PlainText[0] + PlainBlockStart), (PlainTextLength - PlainBlockStart));
	memcpy(Block, (&PlainText[0] + PlainBlockStart),
...
VOID AES_CBC_Decrypt (
    ...
    INOUT UINT *PlainTextLength)
{
    UINT PaddingSize = 0, PlainBlockStart = 0, CipherBlockStart = 0, PlainBlockSize = 0;
    UINT Index = 0;
    ...
    if ((CipherTextLength % AES_BLOCK_SIZES) != 0) {
    	debug(DEBUG_ERROR, "AES_CBC_Decrypt: cipher text length is %d bytes, it can't be divided with no remainder by block size(%d).\n",
            CipherTextLength, AES_BLOCK_SIZES);
        return;
    }
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "AES_CBC_Decrypt: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return;
    }
    if (IVLength != AES_CBC_IV_LENGTH) {
    	debug(DEBUG_ERROR, "AES_CBC_Decrypt: IV length is %d bytes, it must be %d bytes(128bits).\n",
            IVLength, AES_CBC_IV_LENGTH);
        return;
    }
    ...
    {
    	debug(DEBUG_TRACE, "PlainBlockSize=%u, PlainTextLength=%u, PlainBlockStart=%u\n",
    ...
    *PlainTextLength = PlainBlockStart - PaddingSize;
	debug(DEBUG_TRACE, "%s PlainTextLength=%u, PlainBlockStart=%u, PaddingSize=%u\n",__func__,
			*PlainTextLength, PlainBlockStart, PaddingSize);
    ...
    OUT UINT *CipherTextLength)
{
     ...
     */
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "AES_Key_Wrap: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return -1;
    }

//	os_alloc_mem(NULL, (UCHAR **)&pResult, sizeof(UINT8)*PlainTextLength);
	pResult = (UINT8* )malloc(sizeof(UINT8)*PlainTextLength);
    if (pResult == NULL) {
		PLen = sizeof(UINT8)*PlainTextLength;
    	debug(DEBUG_ERROR, "AES_Key_Wrap: allocate %d bytes memory failure.\n", PLen);
        return -2;
    }
...
//    NdisMoveMemory(IV, Default_IV, AES_KEY_WRAP_IV_LENGTH);
    memcpy(IV, Default_IV, AES_KEY_WRAP_IV_LENGTH);

//    NdisMoveMemory(pResult, PlainText, PlainTextLength);
    memcpy(pResult, PlainText, PlainTextLength);
...
//            NdisMoveMemory(Block_Input, IV, 8);
			memcpy(Block_Input, IV, 8);

//            NdisMoveMemory(Block_Input + 8, pResult + (Index_i*8), 8);
			memcpy(Block_Input + 8, pResult + (Index_i*8), 8);
...
//            NdisMoveMemory(pResult + (Index_i*8), (Block_B + 8), 8);
            ...
            memcpy(pResult + (Index_i*8), (Block_B + 8), 8);
        }
    }
...
//    NdisMoveMemory(CipherText, IV, AES_KEY_WRAP_IV_LENGTH);
    ...
    memcpy(CipherText, IV, AES_KEY_WRAP_IV_LENGTH);
//    NdisMoveMemory(CipherText + AES_KEY_WRAP_IV_LENGTH, pResult, PlainTextLength);
    memcpy(CipherText + AES_KEY_WRAP_IV_LENGTH, pResult, PlainTextLength);
    ...
    OUT UINT *PlainTextLength)
{
    ...
    if ((KeyLength != AES_KEY128_LENGTH) && (KeyLength != AES_KEY192_LENGTH) && (KeyLength != AES_KEY256_LENGTH)) {
    	debug(DEBUG_ERROR, "AES_Key_Unwrap: key length is %d bytes, it must be %d, %d, or %d bytes(128, 192, or 256 bits).\n",
            KeyLength, AES_KEY128_LENGTH, AES_KEY192_LENGTH, AES_KEY256_LENGTH);
        return -1;
    }
//	os_alloc_mem(NULL, (UCHAR **)&pResult, sizeof(UINT8)*PlainLength);
	pResult = (UINT8* )malloc(sizeof(UINT8)*PlainLength);
    if (pResult == NULL) {
		Plen = sizeof(UINT8)*PlainLength;
    	debug(DEBUG_ERROR, "AES_Key_Unwrap: allocate %d bytes memory failure.\n", Plen);
        return -2;
    }
...
//    NdisMoveMemory(pResult, CipherText + AES_KEY_WRAP_IV_LENGTH, PlainLength);
    memcpy(IV, CipherText, AES_KEY_WRAP_IV_LENGTH);
    memcpy(pResult, CipherText + AES_KEY_WRAP_IV_LENGTH, PlainLength);
...
//    NdisMoveMemory(PlainText, pResult, PlainLength);
    memcpy(PlainText, pResult, PlainLength);
