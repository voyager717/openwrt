...
enum {
	...
	SERVICE_PRIORITIZATION_COMMAND,
};
...
struct GNU_PACKED akm_suit
{
	unsigned char oui[3];
	unsigned char akm_suit_type;
};



struct GNU_PACKED akm_suite_cap
{
	unsigned char akm_suite_cnt;
	struct akm_suit *akm_suite_info;
};
...
struct GNU_PACKED dpp_sec_cred {
    unsigned short decrypt_threshold;
    ...
    unsigned short payload_len;
    /* contain connector, csign, netacceesskey in order */
    unsigned char payload[0];
};
...
struct GNU_PACKED dpp_bss_cred {
	...
	unsigned short payload_len;
	/*
	** include 2 connectors in the payload
	** first one is for bh, second one is for fh
	*/
	unsigned char payload[0];
};
