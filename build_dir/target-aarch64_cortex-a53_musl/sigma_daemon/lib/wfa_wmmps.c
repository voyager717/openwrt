...
void WfaStaResetAll()
{
	...
	if (num_retry > MAXRETRY) {
		...
		wSENDTO(psSockfd, psTxMsg, msgsize, 0, (struct sockaddr *)&wmmps_info.psToAddr,
		...
		while (gtgWmmPS > 0) {
			...
			if (resetsnd) // When the testcase ends, WfaStaResetAll() sets the resetsnd to 1
			{
				...
				if (psSockfd > 0) {
					...
					rbytes = 0;
					rbytes = recvfrom(psSockfd, (char *)psRxMsg, WMMPS_MSG_BUF_SIZE - 4, 0, &from,
							  (socklen_t *)&len);
				}
				...
				{
					DPRINT_INFO(WFA_OUT,
						    "wfa_wmmps_thread::sta_test(tst case#)=%d rbytes=%d rcv_state=%d "
						    "rmsg[10]=%d\n",
					...
					if (psRxMsg[10] == APTS_STOP) {
						if (wmmps_info.rcv_state > 0) {
							...
							resetsnd = 1; // by Qiuming word, benz, use STOP as super state
								      // to stop test case.
					...
					if (func.statefunc != NULL) {
						...
						DPRINT_INFO(WFA_OUT,
							    "wfa_wmmps_thread::Detected NULL func pt in rcv func table "
							    "vcs_state=%d\n",
							    wmmps_info.rcv_state);
					}
					rbytes = 0;
			...
			if (num_hello > MAXHELLO) {
				...
				DPRINT_INFO(
				    WFA_OUT,
				    "wfa_wmmps_thread::sent hello too many %d. Restart test script or dut/ca in STA\n",
				    num_hello);
	...
	while (gtgWmmPS > 0 && psSockfd > 0) {
		if (wmmps_info.rcv_state > 0) /*  check state forwarded */
		{
			...
			if (curr_state.statefunc != NULL) {
				curr_state.statefunc(curr_state.pw_offon, curr_state.sleep_period,
						     &(state_num)); // send functions
