...
static int tableDscpToTos[15][2] = {{0, 0},    {8, 32},	  {10, 40},  {14, 56},	{18, 72},
				    {22, 88},  {24, 96},  {28, 112}, {34, 136}, {36, 144},
				    {38, 152}, {40, 160}, {46, 184}, {48, 192}, {56, 224}};
...
int wfaSendShortFile(int mySockfd, int streamid, BYTE *sendBuf, int pksize, BYTE *aRespBuf, int *aRespLen)
{
	...
	if (mySockfd == -1) {
		...
		if (myStream != NULL) {
			...
			wfaEncodeTLV(WFA_TRAFFIC_AGENT_SEND_RESP_TLV, sizeof(dutCmdResponse_t), (BYTE *)&sendResp,
				     aRespBuf);
...
int wfaSendBitrateData(int mySockfd, int streamId, BYTE *pRespBuf, int *pRespLen)
{
	...
	if ((mySockfd < 0) || (streamId < 0) || (pRespBuf == NULL) || (pRespLen == NULL)) {
		// DPRINT_INFO(WFA_OUT, "wfaSendBitrateData pass-in parameter err mySockfd=%i streamId=%i pRespBuf=0x%x
	...
	if (theProf->rate == 0 ||
	    theProf->duration == 0) { /*  rate == 0 means unlimited to push data out which is not this routine to do */
	...
	if ((theProf->rate > 3500) || (theProf->pksize > 1000)) {
		DPRINT_INFO(WFA_OUT, "wfaSendBitrateData Warning, must set total streams rate<=10000 and "
				     "payload<=1000; stream bitrate may not meet\n");
	}
	...
	while (runLoop) {
		...
		for (i = 0; i <= (theProf->rate); i++) {
			...
			if (bytesSent != -1) {
				...
				DPRINT_INFO(WFA_OUT,
					    "wfaSendBitrateData wfaTrafficSendTo call ERR counter=%i i=%i; send busy, "
					    "sleep %i MilSec then send\n",
		...
		if (difftime < 0 || difftime >= 1000000) { /* over time used, no sleep, go back to send */
			nOverTimeCount++;
			// if (difftime > 1200000 )
		...
		if (extraTimeSpendOnSending > 0) {
			if (extraTimeSpendOnSending >
			    1000000 - difftime) { /* reduce sleep time to catch up over all on time sending   */
		...
		if (1000 - difftime >
		    0) { /*  only sleep if there is extrac time with in the second did not spend on sending */
