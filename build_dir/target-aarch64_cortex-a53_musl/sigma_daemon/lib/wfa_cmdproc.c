...
int xcCmdProcStaP2pStartGroupFormation(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	wfaEncodeTLV(WFA_STA_P2P_START_GRP_FORMATION_TLV, sizeof(caStaP2pStartGrpForm_t), (BYTE *)staP2pStartGrpForm,
		     aBuf);
...
int xcCmdProcStaSendP2pPresenceReq(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	wfaEncodeTLV(WFA_STA_P2P_SEND_PRESENCE_REQ_TLV, sizeof(caStaSendP2pPresenceReq_t),
		     (BYTE *)staSendP2pPresenceReq, aBuf);
...
int xcCmdProcStaBlockICMPResponse(char *pcmdStr, BYTE *aBuf, int *aLen)
{
	...
	wfaEncodeTLV(WFA_STA_P2P_BLOCK_ICMP_RESPONSE_TLV, sizeof(caStaBlockICMPResponse_t),
		     (BYTE *)staBlockICMPResponse, aBuf);
	...
	for (;;) {
		...
		} else if (strcasecmp(str, "program") == 0) // VHT or 11n or Voice
		{
			...
			if (strcasecmp(staWirelessParams->program, "VHT") == 0) {
				// process addba_reject, ampdu, amsdu, stbc_rx, width, smps, txsp_stream, rxsp_stream,
				// band, DYN_BW_SGNL SGI80, TXBF, LDPC, Opt_md_notif_ie, nss_mcs_cap, tx_lgi_rate,
				// zero_crc, vht_tkip, vht_wep, bw_sgnl
			}
		}
	}
			...
			if (strcasecmp(str, "Advertise") == 0) {
				...
				for (;;) {
					...
					if (strcasecmp(str, "service_name") == 0) {
						...
						if (strcasecmp(str, "OOB") == 0) {
							strcpy(
							    staInvokeCmd->InvokeCmds.primtiveType.AdvPrim.serviceName,
							    "OOB");
						} else {
							strncpy(
							    staInvokeCmd->InvokeCmds.primtiveType.AdvPrim.serviceName,
							    str, 63);
							staInvokeCmd->InvokeCmds.primtiveType.AdvPrim.serviceName[63] =
							    '\0';
						}
						...
						str = strtok_r(NULL, ",", &pcmdStr);
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.AdvPrim.serviceInfo, str,
							63);
				...
				for (;;) {
					...
					if (strcasecmp(str, "service_name") == 0) {
						...
						if (strcasecmp(str, "OOB") == 0) {
							strcpy(
							    staInvokeCmd->InvokeCmds.primtiveType.SeekPrim.serviceName,
							    "OOB");
						} else {
							strncpy(
							    staInvokeCmd->InvokeCmds.primtiveType.SeekPrim.serviceName,
							    str, 63);
							staInvokeCmd->InvokeCmds.primtiveType.SeekPrim.serviceName[63] =
							    '\0';
						}
						...
						str = strtok_r(NULL, ",", &pcmdStr);
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.SeekPrim.macaddress, str,
						...
						str = strtok_r(NULL, ",", &pcmdStr);
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.SeekPrim.serviceInfo, str,
							63);
				...
				for (;;) {
					...
					if (strcasecmp(str, "service_mac") == 0) {
						...
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.ConnSessPrim.serviceMac,
						...
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.ConnSessPrim.sessionInfo,
							str, 63);
						...
						str = strtok_r(NULL, ",", &pcmdStr);
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.ConnSessPrim.ssid, str,
							63);
				...
				for (;;) {
					...
					if (strcasecmp(str, "session_mac") == 0) {
						...
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.ConfSessPrim.sessionMac,
							str, WFA_MAC_ADDR_STR_LEN - 1);
				...
				for (;;) {
					...
					if (strcasecmp(str, "session_mac") == 0) {
						str = strtok_r(NULL, ",", &pcmdStr);
						staInvokeCmd->InvokeCmds.primtiveType.SetSessRdyPrim.sessionMac_flag =
						    1;
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.SetSessRdyPrim.sessionMac,
							str, WFA_MAC_ADDR_STR_LEN - 1);
				...
				for (;;) {
					...
					if (strcasecmp(str, "session_mac") == 0) {
						...
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.BoundPortPrim.sessionMac,
							str, WFA_MAC_ADDR_STR_LEN - 1);
				...
				for (;;) {
					...
					if (strcasecmp(str, "advId") == 0) {
						...
						str = strtok_r(NULL, ",", &pcmdStr);
						staInvokeCmd->InvokeCmds.primtiveType.ServStatusChngPrim.serviceStatus =
						    atoi(str);
					}
				}
				...
				for (;;) {
					...
					if (strcasecmp(str, "session_mac") == 0) {
						...
						strncpy(staInvokeCmd->InvokeCmds.primtiveType.CloseSessPrim.sessionMac,
							str, WFA_MAC_ADDR_STR_LEN - 1);
				...
				for (;;) {
					...
					if (strcasecmp(str, "service_mac") == 0) {
						...
						strncpy(staManageServCmd->MngCmds.MgtServ.serviceMac, str,
							WFA_MAC_ADDR_STR_LEN - 1);
						staManageServCmd->MngCmds.MgtServ.serviceMac[WFA_MAC_ADDR_STR_LEN - 1] =
						    '\0';
					} else if (strcasecmp(str, "AdvID") == 0) {
						str = strtok_r(NULL, ",", &pcmdStr);
						...
						for (str2 = str;; str2 = NULL) {
							...
							if (subtoken == NULL)
								break;
							strncpy(staManageServCmd->MngCmds.MgtServ.modFileList[index],
								str, 16);
