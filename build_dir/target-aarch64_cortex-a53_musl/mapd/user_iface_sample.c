...
#define ETH_ONBOARDING_STATUS_NOTIF					7
#define SEC_CONNECTION_FAILURE					11
#define USER_FAIL_EVENTS						12
...
#endif /*VENDOR1_FEATURE_EXTEND*/
struct mapd_user_conn_fail_event
{
	unsigned char reason[64];
};

struct mapd_user_fail_event
{
...
int main(int argc, char *argv[])
{
	...
	if (os_snprintf(socket_path, sizeof(socket_path), "%s", argv[1]) < 0) {
		printf("Error in os_snprintf: line %d\n", __LINE__);
		return -1;
	}
	...
	while (1) {
		...
		switch (mapd_event->event_id)
		{
			...
			case WIRELESS_CLIENT_JOIN_NOTIF:
			{
				...
				printf("Received join for "MACSTR"\n",
					MAC2STR(client_event->sta_mac));
				printf("BSSID = "MACSTR"\n",
					MAC2STR(client_event->bssid));
				printf("SSID = %s\n", client_event->ssid);
				break;
			}
			case WIRELESS_CLIENT_LEAVE_NOTIF:
			{
				struct mapd_user_iface_wireless_client_event *client_event;
				client_event = (struct mapd_user_iface_wireless_client_event *)mapd_event->event_body;
				printf("Received leave for "MACSTR"\n",
					MAC2STR(client_event->sta_mac));
				printf("BSSID = "MACSTR"\n",
					MAC2STR(client_event->bssid));
				printf("SSID = %s\n", client_event->ssid);
			...
			case CH_PREF_NOTIF:
			{
				...
				printf("Channel Preference Report!!\nRadio ID "MACSTR"\n",
					MAC2STR(client_event->radio_id));
				printf("ALMAC = "MACSTR"\nPreference: %d\n",
					MAC2STR(client_event->almac), client_event->perference);
				printf("Channel list: ");
			...
			case TRIGGER_MAP_STEERING_NOTIF:
			{
				...
				printf("MAP steering STA "MACSTR"\n",
					MAC2STR(client_event->sta_mac));
				printf("SSID = %s\n", client_event->ssid);
			...
			case GET_BEACON_REPORT_NOTIF:
			{
				...
				printf("MAP steering STA "MACSTR"\n",
					MAC2STR(client_event->sta_mac));
				printf("SSID = %s\n", client_event->ssid);
				printf("Current AP BSSID = "MACSTR"\n",
					MAC2STR(client_event->bssid));
				printf("Current AP DL RSSI:%ddbm\n", client_event->dl_rssi);
				printf("Target AP BSSID = "MACSTR"\n",
					MAC2STR(client_event->target_bssid));
...
#endif /*VENDOR1_FEATURE_EXTEND*/
			case SEC_CONNECTION_FAILURE:
			{
				struct mapd_user_conn_fail_event *conn_evt;
				conn_evt = (struct mapd_user_conn_fail_event *)mapd_event->event_body;
				printf("REASON: %s\n", conn_evt->reason);
			}
			case USER_FAIL_EVENTS:
			{
				struct mapd_user_fail_event *conn_evt;
				conn_evt = (struct mapd_user_fail_event *)mapd_event->event_body;
				printf("REASON: %s, ID: %d\n", conn_evt->reason, conn_evt->reason_id);
			}
		}
	}
	return 0;
