...
#define MAX_BUFF_DYNAMIC_INC_CMD 50
...
enum {
	...
	WAPP_USER_SEND_DPP_CCE_INDICATION_FRAME, //Prakhar
	WAPP_USER_GET_ASSOC_WIFI6_STA_STATUS, //MAP_R3_WF6
...
struct GNU_PACKED channel_report_wapp
{
	u8 AutoChannelSkipListNum;
	u8 AutoChannelSkipList[MAX_NUM_OF_CHANNELS + 1];
#ifdef MAP_R4_SPT
	unsigned char spt_rep_num;
	struct ap_spt_reuse_resp spt_rep_info[3];
#endif
	unsigned char ch_rep_num;
	struct ch_rep_info info[0];
};
...
struct GNU_PACKED conn_fail_notif {
	char reason[64];
};
...
struct GNU_PACKED user_fail_notif {
...
struct GNU_PACKED wapp_bhsta_info {
	u8 mac_addr[ETH_ALEN];
...
struct GNU_PACKED map_spt_reuse_req{
	unsigned char spt_req_cnt;
	struct ap_spt_reuse_req spt_reuse_report[0];
};
