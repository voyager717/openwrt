...
int eloop_init(void)
{
	...
	if (eloop.epollfd < 0) {
		mapd_printf(MSG_ERROR, "%s: epoll_create1 failed. %s",
	...
	if (eloop.kqueuefd < 0) {
		mapd_printf(MSG_ERROR, "%s: kqueue failed: %s",
			   __func__, strerror(errno));
		return -1;
	}
#endif /* CONFIG_ELOOP_KQUEUE */
...
static int eloop_sock_queue(int sock, eloop_event_type type)
{
	...
	if (epoll_ctl(eloop.epollfd, EPOLL_CTL_ADD, sock, &ev) < 0) {
		mapd_printf(MSG_ERROR, "%s: epoll_ctl(ADD) for fd=%d failed: %s",
...
static int eloop_sock_queue(int sock, eloop_event_type type)
{
	...
	if (kevent(eloop.kqueuefd, &ke, 1, NULL, 0, NULL) == -1) {
		mapd_printf(MSG_ERROR, "%s: kevent(ADD) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return -1;
	}
	return 0;
                                     ...
                                     void *eloop_data, void *user_data)
{
	...
	if (eloop.count + 1 > eloop.epoll_max_event_num) {
		...
		if (temp_events == NULL) {
			mapd_printf(MSG_ERROR, "%s: malloc for epoll failed: %s",
				   __func__, strerror(errno));
			return -1;
		}
                                         ...
                                         int sock)
{
	...
	if (table == NULL || table->table == NULL || table->count == 0 || sock < 0)
		return;
	...
	if (epoll_ctl(eloop.epollfd, EPOLL_CTL_DEL, sock, NULL) < 0) {
		mapd_printf(MSG_ERROR, "%s: epoll_ctl(DEL) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return;
	}
	...
	if (kevent(eloop.kqueuefd, &ke, 1, NULL, 0, NULL) < 0) {
		mapd_printf(MSG_ERROR, "%s: kevent(DEL) for fd=%d failed: %s",
			   __func__, sock, strerror(errno));
		return;
	}
	os_memset(&eloop.fd_table[sock], 0, sizeof(struct eloop_sock));
...
static void eloop_sock_table_destroy(struct eloop_sock_table *table)
{
	if (table) {
		int i;
		for (i = 0; i < table->count && table->table; i++) {
			mapd_printf(MSG_INFO, "ELOOP: remaining socket: "
...
void eloop_get_remaining_timeout(struct os_reltime *rem_time,
			   eloop_timeout_handler handler,
			   void *eloop_data, void *user_data)
{
	...
	dl_list_for_each(tmp, &eloop.timeout, struct eloop_timeout, list) {
		if (tmp->handler == handler &&
		    ...
		    if(os_reltime_before(&now,&tmp->time))
		    	os_reltime_sub(&tmp->time, &now, rem_time);
			else
				rem_time->sec = rem_time->usec = 0;
		}
	}
}
			   ...
			   void *eloop_data, void *user_data)
{
	...
	if (timeout->time.sec < now_sec) {
		 ...
		 */
		mapd_printf(MSG_DEBUG, "ELOOP: Too long timeout (secs=%u) to "
...
int eloop_deplete_timeout(unsigned int req_secs, unsigned int req_usecs,
			  ...
			  void *user_data)
{
	struct os_reltime now = {0, 0}, requested, remaining;
	struct eloop_timeout *tmp;
...
{
	mapd_printf(MSG_ERROR, "%s: eloop: could not process SIGINT or SIGTERM in Killing program forcefully %s",
			   __func__, strerror(errno));
	exit(1);
}
...
static void eloop_handle_signal(int sig)
{
	...
	if ((sig == SIGINT || sig == SIGTERM) && !eloop.pending_terminate) {
		...
		s = signal(SIGALRM, eloop_handle_alarm);
		if (s == SIG_ERR)
			mapd_printf(MSG_ERROR, "%s: signal error %s", __func__, strerror(errno));
		alarm(2);
	}
#endif /* CONFIG_NATIVE_WINDOWS */
			  ...
			  void *user_data)
{
	...
	eloop.signal_count++;
	eloop.signals = tmp;
	if (signal(sig, eloop_handle_signal) == SIG_ERR) {
		err("signal error\n");
		return -1;
	}
	return 0;
...
int eloop_register_signal_terminate(eloop_signal_handler handler,
				    void *user_data)
{
	int ret = 0;//eloop_register_signal(SIGINT, handler, user_data);
	if (ret == 0)
...
void eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		if (res < 0 && errno != EINTR && errno != 0) {
			mapd_printf(MSG_ERROR, "eloop: %s: %s",
#ifdef CONFIG_ELOOP_POLL
				   "poll"
#endif /* CONFIG_ELOOP_POLL */
#ifdef CONFIG_ELOOP_SELECT
...
void eloop_destroy(void)
{
	...
	dl_list_for_each_safe(timeout, prev, &eloop.timeout,
		...
		mapd_printf(MSG_INFO, "ELOOP: remaining timeout: %d.%06d "
